...
struct ChannelPlots {
    ...
    std::array<TH2D*, kNPatch> wave_cor{};
...
ChannelPlots MakeChannelPlots(const std::string& label, const TBcid& cid) {
    ...
    for (int i = 0; i < kNPatch; ++i) {
        ch.wave_cor[i] = new TH2D(
            Form("%s_wave_cor_patch_%d", ch.safe.c_str(), i),
            Form("%s patch %d;Waveform bin;ADC", label.c_str(), i),
            1000, 0, 1000, 200, -100, 100);
...
int main(int argc, char** argv) {
    ...
    for (int iEvt = 0; iEvt < fMaxEvent; ++iEvt) {
        ...
        for (auto& ch : towerChannels) {
            ...
            for (int sample = kWaveFirst; sample < kWaveLastExclusive; ++sample) {
                ...
                for (int patch = 0; patch < kNPatch; ++patch) {
                    ch.wave_cor[patch]->Fill(sample - 1, wavecor_all_patch[patch].at(sample));
                }
    ...
    for (auto& ch : towerChannels) {
        ...
        for (int patch = 0; patch < kNPatch; ++patch) {
            Save2DAndProfile(ch.wave_cor[patch], chDir, chPlotDir);
        }
