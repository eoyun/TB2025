...
int main(int argc, char** argv) {
    ...
    std::string outFile = "./LC_cor/drs_stop_Run_" + std::to_string(fRunNum) + ".root";
    ...
    int LC_num[] = {11, 12, 13, 19, 2, 4, 8, 10, 3, 5, 7, 9, 14, 15, 16, 20 };

    std::vector<TBcid> LC_collector;
    
    std::array<TH2D*, 16> LC_hist{};
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        LC_mean_hist[i] = new TH2D(
            Form("LC%d_mean", LC_num[i]),
            ";DRS cell;#mu - ADC",
            kNumCells, 0, kNumCells,
    ...
    TBread<TBwaveform> readerWave = TBread<TBwaveform>(fRunNum, fMaxEvent, fMaxFile, false, "/pnfs/knu.ac.kr/data/cms/store/user/sungwon/2025_DRC_TB_Data/", {9, 10, 12, 18});
    ...
    for (int iEvt = 0; iEvt < fMaxEvent; iEvt++) {
        ...
        for (int i = 0; i < 16; ++i) {
            TBwaveform LC_tmp = aEvent.GetData(LC_collector.at(i));

            const int drs_stop_LC = LC_tmp.drs_stop();

            const std::vector<short> waveform_LC = LC_tmp.waveform();
