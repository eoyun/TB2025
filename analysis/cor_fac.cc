...
int main(int argc, char** argv) {
    ...
    std::string outFile = "./Cor/drs_stop_Run_" + std::to_string(fRunNum) + ".root";
    ...
    for (int i=0; i< 36; i++){
      ...
      S_hist[i] = new TH2D(Form("M%d_T%d_S",i%9 + 1,i/9 + 1),"",1024,0,1024,200,-100,100);
      C_hist[i] = new TH2D(Form("M%d_T%d_C",i%9 + 1,i/9 + 1),"",1024,0,1024,200,-100,100);
    }
	...
	for (int i=0;i<36;i++){
	  ...
	  std::vector<float> waveform_S = S_tmp.pedcorrectedWaveform();
	  std::vector<float> waveform_C = C_tmp.pedcorrectedWaveform();
          if (i != 31){
